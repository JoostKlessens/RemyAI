...
| GAP-18 | ⬜ | **De rest van WS1 is nog steeds niet toegepast.** `ASSEMBLY.md:218-220` noemt de volledige prijs van het onderzoek: "26 colour values per scheme, five `fontFamily` entries, one `typeScale.button` family, three `radii` values, two `@expo-google-fonts` packages added and one removed. No new component." Alleen de kleuren zijn nu gedaan. De typografie staat nog op Archivo + IBM Plex Mono en de radii op 0/4/8/16/999. Gegeven dat 26 van 26 kleuren gemist waren, is aannemen dat de rest wél geland is precies de fout die dit item bestaat om te voorkomen — en WS2 t/m WS6 (layout, copy, iconen, motion, sociale laag) zijn nog helemaal niet nagelopen |
...
| GAP-20 | ⬜ | **Twaalf vastgelegde haptics zijn nooit bedraad.** `expo-haptics` is geïnstalleerd en wordt in drie bestanden gebruikt; WS5 §3.2 heeft een tabel van vijftien gebeurtenissen. Ontbreken onder meer: het cijfer in `OutcomeCard` (WS5: "the highest-value single change in this report per line of code"), `Ja` op Kiezen, stap vooruit in kookmodus, afvinken op de boodschappenlijst, chipselectie, `SegmentedControl`, allergenen bevestigen, en beide import-uitkomsten. Elk is één regel tegen een dependency die er al staat |
| GAP-21 | ⬜ | **Het einde van kookmodus is stil.** WS5 §4.5 noemt het afvinken van een gerecht "the emotional peak of the whole product" en schrijft drie dingen voor: `notificationAsync(Success)`, een `positive` haarlijn met `transformOrigin: 'left'`, en `impactAsync(Medium)` bij het cijfer. Geschat op drie regels code. `OutcomeCard.tsx` importeert `expo-haptics` niet eens |
| GAP-22 | ⬜ | **De vaste timerbalk in kookmodus is nooit gebouwd — een halve reparatie.** WS5 §4.3 vroeg om een balk die toont dat er een timer loopt op een andere stap, aantikbaar om terug te gaan: "the whole reason reading ahead becomes safe". De state is gehoist (timers overleven navigatie, `cook/[mealId].tsx:183`), maar `TimerDisplay` rendert alleen binnen `stepBlock` en alleen als de huidige stap zelf een duur heeft. Blader naar een stap zonder tijd en een lopende sudderklok is onzichtbaar |
| GAP-23 | ⬜ | **Het importlaadblok flikkert bij een cachetreffer.** `beginLoading` (`paste.tsx:329-342`) zet de laadfase synchroon, maar een cachetreffer keert binnen ~150 ms terug — het hele blok verschijnt en verdwijnt binnen een vijfde seconde. WS5 §5.2 heeft de regel al ("below `durationNormal`, show nothing at all") maar die is nooit op de checkpointlijst toegepast. **Uitgestelde start, geen minimale toondrempel:** een drempel verzint wachttijd bij een eerlijk snel antwoord, precies wat de copy van dit scherm weigert. Dit is een bug, geen polish |
