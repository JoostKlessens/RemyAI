...
| IMP-05 | 🔒 | `DEFAULT_GEMINI_MODEL` in `callExtractionModel.ts` staat op `'gemini-3.6-flash'`, een zwevende alias. Wacht op één snapshot-ID van de eigenaar |
...
| GAP-10 | ⬜ | **`import_throttled` brak de kernregel van `ImportResult` zonder dat één comment het zei.** De union heeft tien varianten, waarvan er twee géén `platform` dragen — `unsupported_url` en `import_throttled` — maar `importResult.ts:71-74` ("one sentence, one exception"), `:476-477` ("THE ONE VARIANT WITH NO `platform`") en `parseImportResult.ts:42-43` en `:510` beweren alle nog dat er één uitzondering is. `:487`, `:122` en `parseImportResult.ts:521` zijn wél bijgewerkt, dus dit is half werk, geen ontwerp. De regel op `:119-124` eist dat een nieuwe variant zich uit de regel argumenteert; `import_throttled`'s eigen doccomment doet dat nergens. Gevonden tijdens GAP-07, maar veroorzaakt door IMP-06/IMP-10 |
| GAP-11 | ⬜ | **Verouderde tellingen in comments, gevonden naast GAP-10.** `index.ts:303-306` noemt "nine sibling modules" en somt er negen op; het zijn er elf. `importResult.ts:44-45` zegt dat vijf modules onder `parse-recipe/` `types.ts` importeren; het zijn er acht. Plus een cluster van ~12 plekken (`canonicalRecipeStore.ts`, `fetchSourceText.ts`, `importRequest.ts`, `htmlJsonLd.ts`, `displayOnlyPolicy.ts`, `importTelemetry.ts` e.a.) dat nog beweert dat niets `supabase/functions/` controleert — sinds `check:functions` is de typekant wél gedekt, de ESLint-kant niet (zie OPS-09). Losse comment-edits, geen code |
